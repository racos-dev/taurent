// Focused tests for the mobile automation mock bridge.
//
...
import { createMobileTauriBridge } from './mockMobileBridge';
...
function getAutomation(): AutomationControl {
  ...
  if (!w.__TAURENT_AUTOMATION__) {
    throw new Error('Expected mockMobileBridge to register a window automation control');
  }
...
describe('mockMobileBridge', () => {
  let automation: AutomationControl;
  const bridge = createMobileTauriBridge();
  ...
  describe('qBClient.workspaceView (P2.5)', () => {
    ...
    it('setWorkspaceView returns a snake_case WorkspaceView with sorted_hashes and facets', async () => {
      const view = await bridge.qBClient.setWorkspaceView(makeRequest());
      ...
      // total_count must match the mock's torrent count.
    ...
    it('getWorkspaceView returns the static view before any setWorkspaceView call', async () => {
    ...
    it('getWorkspaceView returns the static view after setWorkspaceView', async () => {
      const request = makeRequest();
      await bridge.qBClient.setWorkspaceView(request);
      // After set, the view returned by getWorkspaceView remains the
      // static template — setWorkspaceView only echoes the request_id
      // back to listeners on the emit side; it does not cache a new view.
      const view = await bridge.qBClient.getWorkspaceView();
      expect(view).not.toBeNull();
      expect(view?.is_filtered).toBe(false);
      expect(view?.total_count).toBeGreaterThan(0);
    ...
    it('does not re-emit workspace-view-changed when injectCustomDelta changes the state', async () => {
      ...
      // but the mock no longer recomputes or re-emits the view.
      ...
      // No recompute — the static view is unchanged.
      expect(handler).not.toHaveBeenCalled();
    ...
    it('reset() clears workspace view listeners', async () => {
      ...
      // After reset, the static view is still returned, but listeners
