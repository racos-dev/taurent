...
- **mockTauriPluginStore.ts** — In-memory `Store` class mirroring `@tauri-apps/plugin-store`'s constructor and static `Store.load()` APIs. Uses a shared `Map<string, Map<string, unknown>>` keyed by label. Instance `load()` and `save()` are no-ops.
