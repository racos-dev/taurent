...
- **AddTorrentScreen.tsx** — Add torrent by magnet link or file. Uses `useAddTorrentScreenController` from web-core. Mobile file picker via `pickTorrentFiles`; file uploads attach the persisted Taurent source-file cleanup preference, which is editable in file mode. Renders `AddTorrentScreenBody` (web-ui).
...
- **SettingsScreen.tsx** — App and server settings. Uses `useSettingsScreenController` from web-core. Manages theme, remote preference edits, the Taurent-local source `.torrent` cleanup toggle, server editing, and remote shutdown. Renders the local `MobileSettingsScreenBody` because the mobile settings presentation is intentionally denser than the shared desktop-oriented body.
