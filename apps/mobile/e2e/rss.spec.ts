...
test.describe('mobile rss screen in shell', () => {
  test('renders RSS screen inside the shell and keeps the tab bar visible', async ({ page }) => {
    ...
    // place the RSS screen exposes its counts helper text. Asserting the
    // counts subtitle here locks in the new shared subtitle contract for
    // this flow so a regression to title-only mobile headers is caught.
    await expect(
      page.getByText(/0 feeds, 0 rules/, { exact: true }),
    ).toBeVisible();
