...
| `preferences.rs` | App preferences + capability discovery | `get_preferences`, `set_preferences`, `get_version`, `get_webapi_version`, `get_build_info`, `get_default_save_path`, `shutdown_server`, `get_server_capabilities`. The `get_server_capabilities` command fetches app/API versions and probes search/RSS endpoints, then delegates to `qb_core::capability::resolve_capabilities()` to produce a `CapabilitiesResponse { session_generation, server_id, capabilities }` with tri-state capability flags. |
