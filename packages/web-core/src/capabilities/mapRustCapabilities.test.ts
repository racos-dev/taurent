 ...
 * for supportsSearch, supportsRss, supportsPauseResume, and
 * supportsWebSeedManagement.
 */
...
function makeRustCapabilities(
  ...
  webSeedManagement: CapabilityState = 'unknown'
...
describe('mapRustCapabilitiesToFlags', () => {
  ...
  describe('supportsWebSeedManagement', () => {
    it('maps confirmed → true', () => {
      const rust = makeRustCapabilities('unknown', 'unknown', 'unknown', 'confirmed');
      expect(mapRustCapabilitiesToFlags(rust).supportsWebSeedManagement).toBe(true);
    });

    it('maps unsupported → false', () => {
      const rust = makeRustCapabilities('unknown', 'unknown', 'unknown', 'unsupported');
      expect(mapRustCapabilitiesToFlags(rust).supportsWebSeedManagement).toBe(false);
    });

    it('maps unknown → null', () => {
      const rust = makeRustCapabilities('unknown', 'unknown', 'unknown', 'unknown');
      expect(mapRustCapabilitiesToFlags(rust).supportsWebSeedManagement).toBe(null);
    });
  });
  ...
  describe('hasUnknownCapabilities', () => {
    it('is false when all confirmed', () => {
      const rust = makeRustCapabilities('confirmed', 'confirmed', 'confirmed', 'confirmed');
      expect(mapRustCapabilitiesToFlags(rust).hasUnknownCapabilities).toBe(false);
    });

    it('is false when all unsupported', () => {
      const rust = makeRustCapabilities('unsupported', 'unsupported', 'unsupported', 'unsupported');
      expect(mapRustCapabilitiesToFlags(rust).hasUnknownCapabilities).toBe(false);
    });

    it('is true when any state is unknown', () => {
      const rust = makeRustCapabilities('confirmed', 'unsupported', 'confirmed', 'unknown');
  ...
  describe('all states combined', () => {
    it('handles all confirmed', () => {
      ...
      expect(result.supportsWebSeedManagement).toBe(true);
    ...
    it('handles all unsupported', () => {
      ...
      expect(result.supportsWebSeedManagement).toBe(false);
      expect(result.hasUnknownCapabilities).toBe(false);
    });
    ...
    it('handles all unknown', () => {
      ...
      expect(result.supportsWebSeedManagement).toBe(null);
      expect(result.hasUnknownCapabilities).toBe(true);
    });

    it('handles mixed states', () => {
      const rust = makeRustCapabilities('confirmed', 'unsupported', 'unknown', 'confirmed');
      const result = mapRustCapabilitiesToFlags(rust);
      expect(result.supportsSearch).toBe(true);
      expect(result.supportsRss).toBe(false);
      expect(result.supportsPauseResume).toBe(null);
